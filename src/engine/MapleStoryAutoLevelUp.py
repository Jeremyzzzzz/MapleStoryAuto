...
from src.utils.common import (find_pattern_sqdiff, draw_rectangle, screenshot, nms,
    load_image, get_mask, get_minimap_loc_size_compat, get_player_location_on_minimap,
...
class MapleStoryAutoBot:
    ...
    def __init__(self, args):
        ...
        self.is_player_location_valid = False
    ...
    def load_config(self, cfg):
        ...
        if cfg["bot"]["mode"] == "normal":
            ...
            # If no map is configured, auto-detect it from the minimap header
            # so the correct per-map route is loaded for any map.
            if not map_name:
                try:
                    from src.utils.minimap_ocr import get_minimap_map_name
                    frame = self.capture.get_frame() if self.capture else None
                    if frame is None:
                        raise RuntimeError("no frame for map-name detection")
                    region = cfg["minimap"].get("region", [0, 0, 285, 245])
                    map_name = get_minimap_map_name(frame, region=region) or ""
                    if map_name:
                        logger.info(f"[load_config] Auto-detected map: {map_name}")
                except Exception as e:
                    logger.warning(f"[load_config] Map auto-detect failed: {e}")
    ...
    def get_player_location_by_nametag(self):
        ...
        if score < self.cfg["nametag"]["diff_thres"]:
            ...
            self.is_player_location_valid = True

        if not self.is_player_location_valid:
            return None
    ...
    def run_once(self):
        ...
        if minimap_result is None:
            ...
            # Shrink minimap boundary by one pixel to avoid pixel leaking to
            # minimap — only when a white-border minimap was auto-detected.
            # For a fixed fallback region the crop is already exact.
            if region is None:
                x += 1
                y += 1
                w -= 2
        ...
        if self.cfg["health_monitor"].get("input_full_frame", False):
            health_frame = self.img_frame
        else:
            health_frame = self.img_frame[self.cfg["ui_coords"]["ui_y_start"]:, :]
        self.health_monitor.update_frame(health_frame)
        ...
        loc_player_minimap = get_player_location_on_minimap(
                                ...
                                color_tolerance=self.cfg["minimap"].get(
                                    "player_color_tolerance", 0),
                                min_pixels=self.cfg["minimap"].get(
                                    "player_min_pixels", 4),
                                max_pixels=self.cfg["minimap"].get(
                                    "player_max_pixels", 100))
