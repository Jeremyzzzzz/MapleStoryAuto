...
def imread_cn(path, flags=cv2.IMREAD_COLOR):
    """
    Read an image from a possibly non-ASCII path.

    cv2.imread fails on Unicode paths in some builds (C-style fopen
    internally). Read the bytes with Python's open() (Unicode-safe) and
    decode via cv2.imdecode. Returns None on failure.
    """
    try:
...
def imwrite_cn(path, img):
    ...
    cv2.imwrite fails on Unicode paths in some builds. Encode via
    cv2.imencode and write the bytes with Python's open(). Returns True on
    success.
    """
    try:
...
def load_image(path, mode=cv2.IMREAD_COLOR):
    ...
    # Load image (bytes read supports non-ASCII paths on Windows)
    img = imread_cn(path, mode)
    if img is None:
...
def get_minimap_loc_size_compat(img_frame, fallback_region=None):
    """
    Locate the minimap, tolerant of clients whose minimap has no pure-white
    border (e.g. the Shanda legacy client). Tries the white-border detector
    first; if that fails and a fixed region is provided (minimap.region in
    the config), returns the fixed region. The returned tuple is always
    (x, y, w, h).
    """
    loc = get_minimap_loc_size(img_frame)
    if loc is not None:
        return loc
    if fallback_region is not None:
        x, y, w, h = (int(v) for v in fallback_region)
        return (x, y, w, h)
    return None

# Minimap text hints that must never be used as the map name.
...
def get_minimap_map_name(img_frame, region=None, ocr_engine=None):
    ...
    "勇士部落东入口"). We OCR the whole minimap region and pick the longest
    text line that is not a UI hint (小地图/大地图 buttons, window title).
...
def get_player_location_on_minimap(
    ...
    """
    color = np.asarray(minimap_player_color, dtype=np.int16)
    tolerance = np.asarray(color_tolerance, dtype=np.int16)
    if tolerance.ndim == 0:
        tolerance = np.repeat(tolerance, 3)
    if tolerance.shape != (3,):
        raise ValueError("color_tolerance must be a number or three BGR values")
